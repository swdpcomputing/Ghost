...
/// process 64 bytes
void SHA256B::processBlock(const void* data)
{
  // get last hash
  uint32_t a = m_hash[0];
  uint32_t b = m_hash[1];
...
void SHA256B::add(const void* data, size_t numBytes)
{
...
void SHA256B::getHash(unsigned char buffer[SHA256B::HashBytes])
{
...
std::string SHA256B::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
std::string SHA256B::operator()(const std::string& text)
{
  reset();
